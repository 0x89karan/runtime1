...
impl Job {
    /// Render the fixed task template with the server-stamped date and timestamp. `{date}`
    /// and `{ts}` are the only placeholders; both are generated by the scheduler (trusted),
    /// never caller-supplied.
    ///
    /// R4 (attn.2): `{ts}` exists so a prompt can produce a per-fire-unique filename
    /// (`brief-{date}T{ts}Z.md`) instead of the fixed `brief-{date}.md`, which a second
    /// same-day fire silently truncates. Before this, `{HHMMSS}` in a prompt reached the
    /// model as a LITERAL string — the curator has no clock, no shell, and no time tool —
...
"#;
    ...
    fn cos_prompts_never_interpolate_unvalidated_thread_id_into_a_link() {
            ...
            .filter(|(l, _)| *l == "dev" || *l == "overlay")
        {
            ...
            // ⚠ NARROWED (R3.3): the original rule named 5 entities including `(`/`)`. Measured
            // against the real 07-31 brief, 25 of 37 escaped entities were characters the rule
            // never named (`<`/`>`, em-dash, `$`/`'`) and ZERO were `[`/`]` — the model had
            // generalised to HTML-escaping anything markdown-ish, including its own prose
            // em-dashes. `(`/`)` are safe once `[`/`]` are escaped (a `)` cannot close a link
            // the sender never opened), so only 3 entities are required now. `|` MUST stay: it
            // splits a markdown table row, shifting the Thread cell out of column.
            for entity in ["&#91;", "&#93;", "&#124;"] {
                assert!(
                    raw.contains(entity),
            ...
            // Mutation control, the OTHER direction: the narrowing itself must be present, or a
            // future edit could silently re-widen the rule back to over-escaping. `(`/`)` must
            // NOT appear in the entity-substitution line specifically (they legitimately appear
            // elsewhere in the file, e.g. in prose parentheticals, so this checks the RULE
            // LINE'S own text, not the whole file).
            assert!(
                raw.contains("replace `[` `]` `|` with `&#91;` `&#93;` `&#124;`"),
                "{label}: the neutralisation rule must be narrowed to exactly `[ ] |` — if this \
                 fails because the rule now also names `(`/`)` again, that re-widens over-escaping \
                 R3.3 fixed (measured: 25 of 37 escapes on a real brief were characters the old \
                 rule never named, and 0 were `[`/`]`)"
            );
        }
    }
    ...
    fn job_render_substitutes_ts_alongside_date() {
        ...
        assert!(!rendered.contains("{ts}"), "no literal {{ts}} may survive substitution");
        assert!(!rendered.contains("{date}"), "no literal {{date}} may survive substitution");
    }

    /// R4.1's other half: the SHIPPED prompts actually use `{ts}` in the write_file path and the
    /// KB key. Without this, a prompt could ship with the OLD fixed filename — the exact false
    /// green the plan's own history records: "the original criterion 9 mutation control (revert
    /// to the fixed filename) would have passed either way, because the shipped behaviour WAS
    /// the fixed filename." Mutation-verified: reverting either site below must fail this test.
    #[test]
    fn cos_prompts_use_ts_in_both_the_brief_filename_and_the_kb_key() {
        for (label, raw) in COS_PROMPT_SOURCES
            .iter()
            .filter(|(l, _)| *l == "dev" || *l == "overlay")
        {
            assert!(
                raw.contains("brief-{date}T{ts}Z.md"),
                "{label}: write_file must target a {{ts}}-suffixed filename, not the fixed \
                 brief-{{date}}.md a second same-day fire silently truncates"
            );
            assert!(
                raw.contains("key='{date}T{ts}'"),
                "{label}: the ops:briefs kb_put key must also carry {{ts}} — timestamping only \
                 the file leaves the KB record last-writer-wins on the bare {{date}} key, so \
                 'no content lost' would still be false for the KB side"
            );
        }
    }
